...
ssize_t i2c_read(const I2CDevice *device, unsigned int iaddr, void *buf, size_t len)
{
        ...
        if (write(device->bus, addr, device->iaddr_bytes) != (int)device->iaddr_bytes) {
...
ssize_t i2c_write_addr(const I2CDevice *device, unsigned int iaddr)
{
    //ssize_t remain = len;
    //size_t cnt = 0, size = 0;
    ...
    if (write(device->bus, tmp_buf, device->iaddr_bytes) != (int)device->iaddr_bytes) {
...
ssize_t i2c_write_bytes(const I2CDevice *device, unsigned int iaddr, const void *buf, size_t len)
{
    size_t /*cnt = 0,*/ size = 0;
    const unsigned char *buffer = (const unsigned char *)buf;
    unsigned char delay = GET_I2C_DELAY(device->delay);
    ...
    /* Copy data to tmp_buf */
    memcpy(tmp_buf + device->iaddr_bytes, buffer, len);

    size = device->iaddr_bytes + len;

    if (write(device->bus, tmp_buf, size) != (int)size) {
